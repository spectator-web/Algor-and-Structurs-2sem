...
#                 "data": [1, 1],
#                 "expected_any": True  # флаг, что возможны разные решения
#             },
#             {
...
#                 if 'expected' in test:
#                     # Точное сравнение для случаев с однозначным ответом
#                     self.assertEqual(result, test['expected'])
#                 else:
#                     # Для случаев с множеством решений проверяем корректность
...
#             },
#             {
#                 "name": "Большие числа, но мало элементов",
#                 "data": [100, 200, 300],  # 100≤1? Нет, нарушено условие ai ≤ i
#                 # Пропускаем, т.к. данные не соответствуют условию
#             },
#             {
...
#         # Используем n=1000 для умеренной нагрузки (максимум по условию 40000,
#         # но при полной сумме ~500 млн DP-массив будет огромным).
#         n = 1000

#         # Генерируем данные, удовлетворяющие условию ai ≤ i,
#         # и гарантируем чётность суммы (берём все единицы, n чётное)
#         data = [1] * n  # сумма = n, чётная при чётном n

#         print(f"\nТест с {n} отрезками (все единицы, чётное количество)")
...
#         # Дополнительный тест с максимальными значениями ai = i для n=500
#         n2 = 500
#         data2 = [i + 1 for i in range(n2)]  # ai = i+1? Индексация: a_i = i? В условии a_i ≤ i, при i=1 a1≤1 -> 1
#         # Для i=1 a1=1; i=2 a2≤2 и т.д. Чтобы a_i = i, надо a_i = i, но индексы с 1.
#         # Сгенерируем a_i = i для i от 1 до n2.
#         data2 = [i for i in range(1, n2 + 1)]  # сумма = n2(n2+1)/2
#         print(f"\nТест с {n2} отрезками (ai = i)")
#         # Проверим, возможно ли разбиение (только если n2 mod 4 == 0 или 3)
#         total2 = sum(data2)
#         possible = (total2 % 2 == 0)
#         if possible:
#             result2, exec_time2, mem_peak2 = self.measure_performance(solve, data2, n2)
#             print(f"Время выполнения: {exec_time2:.6f} сек")
#             print(f"Пиковое использование памяти: {mem_peak2 / 1024:.2f} КБ")
#             self.assertLess(exec_time2, 2.0, f"Время выполнения {exec_time2:.2f} сек превышает 2 секунды")
#             self.assertTrue(is_valid_solution(data2, result2), "Решение некорректно")
#         else:
#             print("Сумма нечётная, разбиение невозможно – пропускаем измерение.")
...
#                     # Для этого сначала извлечём исходные данные из input_data
...
#                         else:
#                             # Для точного сравнения допустим, что алгоритм вернул конкретное решение
#                             # (здесь мы просто проверяем, что оно совпадает с ожидаемым)
